...
/**
 * Based on inline_l1_norm
 * we store rgba instead of only rgb in preparation of simd instructions
 * might decrease performance due to a bigger memory footprint
 */
...
public:
  ...
  Kernel(const CommonView& v1, const CommonView& v2, int rows, int cols) : BaseKernel(rows, cols) {
    ...
    v1_.i = (uint8_t*)malloc(rows*cols*4*sizeof(uint8_t));
    v2_.i = (uint8_t*)malloc(rows*cols*4*sizeof(uint8_t));
    for (int y = 0; y < rows; ++y) {
      for (int x = 0; x < cols; ++x) {
