...
/**
 * Based on simd_v4
 * pad arrays to allow aligned loads in working view
 * gradients, weights
 */
...
// 35x40 if WINDOW_SIZE = 35. 40 is due to alignment reasons of simd access
float weights[WINDOW_SIZE * WINDOW_SIZE + 5*WINDOW_SIZE] __attribute__ ((aligned (32))) ;
...
float mcost(KernelView& wv, KernelView& ov, float* fp, int x, int y, int rows, int cols, int sign,
            int qy_start, int qy_end, int qx_start, int qx_end, int lda8){
#ifdef TIME_MCOST
  ...
  __m256i mlda8 = _mm256_set1_epi32(lda8);
  ...
  __m256i qdylda8 = _mm256_set1_epi32(qy_start*lda8);
  ...
  for(int qy = qy_start; qy <= qy_end; ++qy){
    ...
    for(int qx = qx_start; qx <= qx_end; qx += 8){
      ...
      __m256 w = _mm256_load_ps(weights + idx);  // float w = weights[idx];
      ...
      __m128i idx_p0_3_left = _mm_add_epi32(_mm256_castsi256_si128(qdycols), qdx_p0_3);  // omit multiplication by four since we have scale
      __m128i idx_p0_3_right = _mm_add_epi32(idx_p0_3_left, _mm256_castsi256_si128(onesi));  // omit multiplication by four since we have scale
      // [R0,G0,B0,A0, R1,G1,B1,A1, R2,G2,B2,A2, R3,G3,B3,A3]
      // left: left matching point for weighting
      // scale = 4 since RGBA uses 4 bytes
      __m128i RGBA0_RGBA1_RGBA2_RGB3_left = _mm_i32gather_epi32(reinterpret_cast<const int*>(ov.i), idx_p0_3_left, 4);
      ...
      // scale = 4 since RGBA uses 4 bytes
      __m128i RGBA0_RGBA1_RGBA2_RGB3_right = _mm_i32gather_epi32(reinterpret_cast<const int*>(ov.i), idx_p0_3_right, 4);
      ...
      __m256i qdy_lda8_qdx_left = _mm256_add_epi32(qdylda8, qdx);
      __m256i qdy_lda8_qdx_right = _mm256_add_epi32(qdy_lda8_qdx_left, onesi);
      __m256 ovg_left = _mm256_mask_i32gather_ps(zerosf, ov.g, qdy_lda8_qdx_left, mask, 4);
      __m256 ovg_right = _mm256_mask_i32gather_ps(zerosf, ov.g, qdy_lda8_qdx_right, mask, 4);
      __m256 ovg = _mm256_fmadd_ps(wm, ovg_left, _mm256_mul_ps(inv_fac, ovg_right));

      // float iqgnorm = std::abs(wv.g[qy*cols+qx] - ovg);
      __m256 wvg = _mm256_load_ps(wv.g + qy*lda8 + qx);  // potentially loads too much data -> use mask
    ...
    qdylda8 = _mm256_add_epi32(qdylda8, mlda8);
  }
  // accumulators
    ...
    for(int qx = qx_start; qx <= qx_end; qx += 8){
      for (int i = 0; i < 8; ++i) {
        if (qx + i <= qx_end) {
          ...
          int inorm = l1norm_naive(&wv.i[y*cols*4+x*4], &wv.i[qy*cols*4+(qx+i)*4]);
 ...
 * @param cpv   indicates which one is the work view. false: left, false: right
 */
void evaluatePlanesCost(KernelView& wv, KernelView& ov, int rows, int cols, int sign, int lda8){
  for(int y=0; y<rows; ++y)
    for(int x=0; x<cols; ++x){
      ...
      *cc = mcost(wv,ov,fp,x,y,rows,cols,sign, qy_start, qy_end, qx_start, qx_end, lda8);
    }
}
/**
...
void SpatialPropagation(KernelView& wv, KernelView& ov, int x, int y, int rows, int cols, int sign,
                        bool isEven, int qy_start, int qy_end, int qx_start, int qx_end, int lda8){
  int n1x,n1y,n2x,n2y;
  bool n1in,n2in;
  ...
  if(n1in){
    ...
    float cost_new =  mcost(wv,ov,plane_new,x,y,rows,cols,sign, qy_start, qy_end, qx_start, qx_end, lda8);
...
void ViewPropagation(KernelView& wv, KernelView& ov, int x, int y, int rows, int cols, int sign,
  ...
  for (int x_other = 0; x_other < cols; ++x_other) {
      ...
      if (mx != x) {    // no match
        ...
        float* cost_old = &(wv.c[y * cols + x]);
        float  cost_new = mcost(wv, ov, new_plane, x, y, rows, cols, sign, qy_start, qy_end, qx_start, qx_end, lda8);
        if(cost_new < *cost_old){
          //Update the plane
...
void PlaneRefinement(KernelView& wv, KernelView& ov, int x, int y, int rows, int cols, int sign,
  ...
  while(max_dz >= end_dz)
  {
    ...
    float cost_new = mcost(wv,ov,plane,x,y,rows,cols,sign, qy_start, qy_end, qx_start, qx_end, lda8);
 ...
 * @param isEven  indicates if this is an even iteration (decides which neighbors we look at.
 */
void processPixel(KernelView& wv, KernelView& ov, int x, int y, int rows, int cols, int sign, bool isEven, int lda8){
  int HALF_WIN = WINDOW_SIZE/2;
  int qy_start = y - HALF_WIN >= 0 ? y - HALF_WIN : 0;
  int qy_end = y + HALF_WIN < rows ? y + HALF_WIN : rows - 1;
  int qx_start = x - HALF_WIN >= 0 ? x - HALF_WIN : 0;
  int qx_end = x + HALF_WIN < cols ? x + HALF_WIN : cols - 1;
  precompute_weights(wv, x, y, rows, cols, qy_start, qy_end, qx_start, qx_end);
  SpatialPropagation(wv,ov, x,y,rows,cols,sign,isEven, qy_start, qy_end, qx_start, qx_end, lda8);
  ViewPropagation(wv,ov, x,y,rows,cols,sign,isEven, qy_start, qy_end, qx_start, qx_end, lda8);
  PlaneRefinement(wv,ov, x,y,rows,cols,sign,isEven, qy_start, qy_end, qx_start, qx_end, lda8);
}

void process(KernelView& v1, KernelView& v2, int rows, int cols, int lda8) {
  // Eval plane's cost
  // TODO: should this be moved to a helper?
  evaluatePlanesCost(v1, v2, rows, cols, -1, lda8);
  evaluatePlanesCost(v2, v1, rows, cols, 1, lda8);
    ...
    for(int work_view=0; work_view < 2; ++work_view){
      ...
      if(isEven){
        ...
        for(int y=0;y<rows;y++){
          ...
          for(int x=0;x<cols;x++){
            processPixel(wv,ov, x,y,rows,cols,sign,isEven, lda8);
          }
        }
      }else{
        // Bottom up
...
public:
  ...
  Kernel(const CommonView& v1, const CommonView& v2, int rows, int cols) : BaseKernel(rows, cols) {
    // leading dimension of padded matrix (up to multiple of 8)
    // https://stackoverflow.com/questions/3407012/c-rounding-up-to-the-nearest-multiple-of-a-number
    int remainder = cols % 8;
    if (remainder == 0) {
        lda8_ = cols;
    }
    lda8_ = cols + 8 - remainder;
    ...
    // Gradients
    v1_.g = static_cast<float *>(aligned_alloc(32, (rows*lda8_)*sizeof(float)));
    v2_.g = static_cast<float *>(aligned_alloc(32, (rows*lda8_)*sizeof(float)));
    for (int y = 0; y < rows; ++y) {
      for (int x = 0; x < cols; ++x) {
        v1_.g[y * lda8_ + x] = v1.g[y * cols + x];
        v2_.g[y * lda8_ + x] = v2.g[y * cols + x];
      }
    }
    // Planes
  ...
  void run_patch_match() {
    process(v1_, v2_, rows_, cols_, lda8_);
  }
  ...
  std::pair<float, float> test_mcost(int x, int y) {
    ...
    float cost_left = mcost(v1_, v2_, fp, x, y, rows_, cols_, -1, qy_start, qy_end, qx_start, qx_end, lda8_);
    fp = &(v2_.p[(y*cols_*3)+x*3]);
    precompute_weights(v2_, x, y, rows_, cols_, qy_start, qy_end, qx_start, qx_end);
    float cost_right = mcost(v2_, v1_, fp, x, y, rows_, cols_, +1, qy_start, qy_end, qx_start, qx_end, lda8_);
  ...
  float peakperf_mcost(void){
    ...
    return mcost(v2_, v1_, fp, x, y, rows_, cols_, +1, qy_start, qy_end, qx_start, qx_end, lda8_);
  }
  // destructor
  ~Kernel() {
    delete[] v1_.i;
