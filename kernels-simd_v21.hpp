...
/**
 * Based on simd_v16
 * reordered gradient weights to be computed before RGB
 */
...
float mcost(KernelView& wv, KernelView& ov, float* fp, int x, int y, int rows, int cols, int sign,
  ...
  for(int qy = qy_start; qy <= qy_end; ++qy){
    ...
    // TODO try prefetching explicit function every second loop
    for(int qx = qx_start; qx <= qx_end; qx += 8){
      ...
      // TODO move float out to remove conversion back to int
      ...
      ///
      // pixel 0-3
      // TODO still need to do qdy + qdx
      //                       no factor of 4, use as scale factor in gather
      //                       
      ...
      // TODO Convert this to split RGB format
      // left: left matching point for weighting
      //
      // TODO no longer need temp
      // Right = left + 1 or something.
      ...
      __m256 r_ov_left  = _mm256_mask_i32gather_ps(zerosf, ov.ir_f, qdy_cols_qdx_left,  mask, 4);
      __m256 g_ov_left  = _mm256_mask_i32gather_ps(zerosf, ov.ig_f, qdy_cols_qdx_left,  mask, 4);
      __m256 b_ov_left  = _mm256_mask_i32gather_ps(zerosf, ov.ib_f, qdy_cols_qdx_left,  mask, 4);
      ...
      __m256 r_ov_right = _mm256_mask_i32gather_ps(zerosf, ov.ir_f, qdy_cols_qdx_right, mask, 4);
      __m256 g_ov_right = _mm256_mask_i32gather_ps(zerosf, ov.ig_f, qdy_cols_qdx_right, mask, 4);
      __m256 b_ov_right = _mm256_mask_i32gather_ps(zerosf, ov.ib_f, qdy_cols_qdx_right, mask, 4);
      __m256 ov_r = _mm256_fmadd_ps(wm, r_ov_left, _mm256_mul_ps(inv_fac, r_ov_right));
      __m256 ov_g = _mm256_fmadd_ps(wm, g_ov_left, _mm256_mul_ps(inv_fac, g_ov_right));
      __m256 ov_b = _mm256_fmadd_ps(wm, b_ov_left, _mm256_mul_ps(inv_fac, b_ov_right));

      // work view
      __m256 wv_r = _mm256_loadu_ps(wv.ir_f + qy*cols + qx);
      __m256 wv_g = _mm256_loadu_ps(wv.ig_f + qy*cols + qx);
      __m256 wv_b = _mm256_loadu_ps(wv.ib_f + qy*cols + qx);
      ...
      //TODO just multiply by bilinear weights
      ...
      //
      //
      // TODO test similar order as gradients to mask fma behind load of wv
      __m256 vp_r = _mm256_fmadd_ps(wm, r_ov_left, _mm256_fmsub_ps(inv_fac, r_ov_right, wv_r));
      __m256 vp_g = _mm256_fmadd_ps(wm, g_ov_left, _mm256_fmsub_ps(inv_fac, g_ov_right, wv_g));
      __m256 vp_b = _mm256_fmadd_ps(wm, b_ov_left, _mm256_fmsub_ps(inv_fac, b_ov_right, wv_b));

      // TODO x3 instead of 4

      // float iqnorm = std::abs(vp1) + std::abs(vp2) + std::abs(vp3);
      // weighted intensity difference of rgba, absolute value
      __m256 abs_vp_r = _mm256_andnot_ps(minuszeros, vp_r);
      __m256 abs_vp_g = _mm256_andnot_ps(minuszeros, vp_g);
      __m256 abs_vp_b = _mm256_andnot_ps(minuszeros, vp_b);
      ...
      //hadds replaced by just summing across r,g,b

      // Sum up channels to get intensity difference
      __m256 iqnorm = _mm256_add_ps(abs_vp_r, _mm256_add_ps(abs_vp_g, abs_vp_b));
...
public:
  ...
  Kernel(const CommonView& v1, const CommonView& v2, int rows, int cols) : BaseKernel(rows, cols) {
    // TODO update this with split rgb channels
    ...
    v1_.ir_f = (float*)malloc(rows*cols*sizeof(float));
    v1_.ig_f = (float*)malloc(rows*cols*sizeof(float));
    v1_.ib_f = (float*)malloc(rows*cols*sizeof(float));
    v2_.ir_f = (float*)malloc(rows*cols*sizeof(float));
    v2_.ig_f = (float*)malloc(rows*cols*sizeof(float));
    v2_.ib_f = (float*)malloc(rows*cols*sizeof(float));
    for (int y = 0; y < rows; ++y) {
      for (int x = 0; x < cols; ++x) {
        v1_.ir_f[y * cols + x] = v1.i[y * cols * 3 + x * 3 + 0];
        v1_.ig_f[y * cols + x] = v1.i[y * cols * 3 + x * 3 + 1];
        v1_.ib_f[y * cols + x] = v1.i[y * cols * 3 + x * 3 + 2];
      }
    }
    for (int y = 0; y < rows; ++y) {
      for (int x = 0; x < cols; ++x) {
        v2_.ir_f[y * cols + x] = v2.i[y * cols * 3 + x * 3 + 0];
        v2_.ig_f[y * cols + x] = v2.i[y * cols * 3 + x * 3 + 1];
        v2_.ib_f[y * cols + x] = v2.i[y * cols * 3 + x * 3 + 2];
      }
    }
    // Gradients
