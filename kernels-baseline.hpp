...
void PlaneRefinement(KernelView& v1, KernelView& v2, int x, int y, int rows, int cols, bool cpv, bool isEven){
  ...
  // get sign of z-component of normal
  float nz_sign = (plane_old[5] >= 0.f) - (plane_old[5] < 0.f);
  // possibly invert normal, needed for evaluation due to randomness, doesn't change a,b,c
  plane_old[3] *= nz_sign;
  plane_old[4] *= nz_sign;
  plane_old[5] *= nz_sign;
...
public:
  ...
  float peakperf_mcost(void){
    ...
    return mcost(v1_, v2_, fp, x, y, rows_, cols_, false);
  }
  // destructor
  ~Kernel() {
    // nothing since we work directly on the Common View data
