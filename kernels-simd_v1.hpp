...
/**
 * Based on simd_v1
 * first approach of vectorization
 */
...
float weights[WINDOW_SIZE * WINDOW_SIZE + 8];
...
float mcost(KernelView& wv, KernelView& ov, float* fp, int x, int y, int rows, int cols, int sign,
  ...
  __m256 eights = _mm256_set1_ps(8);
  ...
  for(int qy = qy_start; qy <= qy_end; ++qy){
    ...
    __m256 disp = _mm256_fmadd_ps(fp1, qys, _mm256_fmadd_ps(fp0, _mm256_cvtepi32_ps(qxis), fp2));  // reordered to hide latency
    ...
    for(int qx = qx_start; qx <= qx_end; qx += 8){
      ...
      __m256 match = _mm256_fmadd_ps(signs, disp, _mm256_cvtepi32_ps(qxis)); // float match = qx + sign * disp;
      match = _mm256_max_ps(zerosf, _mm256_min_ps(colsm2, match));  // match = match > cols - 2 ? cols - 2 : match < 0 ? 0 : match;
      ...
      // work view
      __m128i RGBA0_RGBA1_RGBA2_RGB3_wv = _mm_loadu_si128(reinterpret_cast<const __m128i*>(wv.i + qy*cols*4 + qx*4));
      ...
      __m256 wm00001111 = _mm256_permutevar8x32_ps(wm, perm0); // AVX2
      __m256 inv_wm00001111 = _mm256_permutevar8x32_ps(inv_fac, perm0); // AVX2
      __m256 wm22223333 = _mm256_permutevar8x32_ps(wm, perm1); // AVX2
      ...
      // work view
      __m128i RGBA4_RGBA5_RGBA6_RGB7_wv = _mm_loadu_si128(reinterpret_cast<const __m128i*>(wv.i + qy*cols*4 + qx*4 + 16));
      ...
      __m256 wm44445555 = _mm256_permutevar8x32_ps(wm, perm2); // AVX2
      __m256 inv_wm44445555 = _mm256_permutevar8x32_ps(inv_fac, perm2); // AVX2
      __m256 wm66667777 = _mm256_permutevar8x32_ps(wm, perm3); // AVX2
      ...
      disp = _mm256_fmadd_ps(fp0, eights, disp);  // disp += fp[0];
...
public:
  ...
  Kernel(const CommonView& v1, const CommonView& v2, int rows, int cols) : BaseKernel(rows, cols) {
    ...
    v1_.i = (uint8_t*)malloc((rows*cols*4 + 8)*sizeof(uint8_t));
    v2_.i = (uint8_t*)malloc((rows*cols*4 + 8)*sizeof(uint8_t));
    for (int y = 0; y < rows; ++y) {
      for (int x = 0; x < cols; ++x) {
    ...
    // Gradients
    v1_.g = (float*)malloc((rows*cols + 8)*sizeof(float));
    v2_.g = (float*)malloc((rows*cols + 8)*sizeof(float));
    for (int y = 0; y < rows; ++y) {
      for (int x = 0; x < cols; ++x) {
