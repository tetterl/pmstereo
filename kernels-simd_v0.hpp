...
/**
 * Based on rgba
 * parts of mcost are vectorized, difficult part is skipped
 */
...
float mcost(KernelView& wv, KernelView& ov, float* fp, int x, int y, int rows, int cols, int sign,
  ...
  for(int qy = qy_start; qy <= qy_end; ++qy){
    ...
    __m256 disp = _mm256_fmadd_ps(fp1, qys, _mm256_fmadd_ps(fp0, _mm256_cvtepi32_ps(qxis_init), fp2));  // reordered to hide latency
    ...
    for(int qx = qx_start; qx <= qx_end; qx += 8){
      ...
      // version without SIMD for weighted l1 norm computation (not efficient)
      // convert to non-simd arrays
      int qdx_[8];
      _mm256_storeu_si256( (__m256i *)qdx_, qdx);
      float mask_[8];
      _mm256_storeu_ps(mask_, mask);
      float wm_[8];
      _mm256_storeu_ps(wm_, wm);
      float inv_fac_[8];
      _mm256_storeu_ps(inv_fac_, inv_fac);
      float iqnorm_[8];

      for (int i = 0; i < 8; ++i) {
        iqnorm_[i] = 0;
        if (std::isnan(mask_[i])) {
          float vp1 =  wm_[i] * ov.i[qy*cols*4+qdx_[i]*4 + 0] + inv_fac_[i] * ov.i[qy*cols*4+qdx_[i]*4 + 4] - wv.i[qy*cols*4+(qx+i)*4 + 0];
          float vp2 =  wm_[i] * ov.i[qy*cols*4+qdx_[i]*4 + 1] + inv_fac_[i] * ov.i[qy*cols*4+qdx_[i]*4 + 5] - wv.i[qy*cols*4+(qx+i)*4 + 1];
          float vp3 =  wm_[i] * ov.i[qy*cols*4+qdx_[i]*4 + 2] + inv_fac_[i] * ov.i[qy*cols*4+qdx_[i]*4 + 6] - wv.i[qy*cols*4+(qx+i)*4 + 2];
          float iqnorm = std::abs(vp1) + std::abs(vp2) + std::abs(vp3);
          iqnorm_[i] = iqnorm;
        }
      }
      // convert back
      __m256 iqnorm = _mm256_loadu_ps(iqnorm_);
...
public:
  ...
  Kernel(const CommonView& v1, const CommonView& v2, int rows, int cols) : BaseKernel(rows, cols) {
    ...
    // expanded array for SIMD since we might access date not in the range [0, rows*cols]
