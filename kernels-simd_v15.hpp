...
/**
 * Based on simd_v14
 * replace sub/add operations by fmadd/fmsub since we have more ports
 *
 */
...
float mcost(KernelView& wv, KernelView& ov, float* fp, int x, int y, int rows, int cols, int sign,
  ...
  for(int qy = qy_start; qy <= qy_end; ++qy){
    ...
    __m256 qdx_floor = _mm256_floor_ps(match); // int qdx = (int)match;
    __m256i qdx = _mm256_cvtps_epi32(qdx_floor); // int qdx = (int)matc
    ...
    for(int qx = qx_start; qx <= qx_end; qx += 8){
      ...
      __m256 wm00001111 = _mm256_fmsub_ps(ones, ones, inv_wm00001111);
      __m256 inv_wm22223333 = _mm256_permutevar8x32_ps(inv_fac, perm1); // AVX2
      __m256 wm22223333 = _mm256_fmsub_ps(ones, ones, inv_wm22223333);
      ...
      __m256 wm44445555 = _mm256_fmsub_ps(ones, ones, inv_wm44445555);
      __m256 inv_wm66667777 = _mm256_permutevar8x32_ps(inv_fac, perm3); // AVX2
      __m256 wm66667777 = _mm256_fmsub_ps(ones, ones, inv_wm66667777);
      ...
      match_unclamped = _mm256_fmadd_ps(ones, match_unclamped, match_increase);
      match = _mm256_max_ps(zerosf, _mm256_min_ps(colsm2, match_unclamped));
      qdx_floor = _mm256_floor_ps(match); // int qdx = (int)match;
      qdx = _mm256_cvtps_epi32(qdx_floor); // int qdx = (int)matc
      inv_fac = _mm256_sub_ps(match, _mm256_cvtepi32_ps(qdx)); // float inv_fac = 1.f - wm = match - qdx;
      wm = _mm256_sub_ps(ones, inv_fac); // float wm = 1.f - (match - qdx) = 1.f - inv_fac;
      idx_left = _mm256_add_epi32(qdycols, qdx);
      idx_left = _mm256_add_epi32(idx_left, idx_left);
      idx_left = _mm256_add_epi32(idx_left, idx_left); // times four
      _mm256_store_si256((__m256i *)idx_left_tmp, idx_left);
