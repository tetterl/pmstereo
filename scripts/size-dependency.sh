...
if [ "$#" -lt 1 ]
  then
    ...
    echo "./size-dependency.sh JOB_NAME RUN_SUBSET kernels..."
    ...
    echo "example:"
    echo "./size-dependency.sh 0 "
    exit 1
fi

RUN_RESULT=../runs/run-size-dependency/$1
...
if [ ${#binaries[@]} -eq 0 ]; then
    ...
    for binversion in "${binaries[@]}"; do
      ...
      for kernel in "${kernellist[@]}"; do
        ...
        for SCALE in 10 15 20 25 30 40 50 60 70 85 100
        do 
