...
if [ "$#" -lt 4 ]
  then
    ...
    echo "./time-all-kernels-final.sh {trainingQ, trainingF} {image name} EVAL RUN_SUBSET"
    ...
    echo "example:"
    echo "./time-all-kernels-final.sh trainingQ Teddy 0 0 "
    exit 1
fi

IMAGE_DIR=$1 #choose trainingF for fullrs
IMAGE_NAME=$2 
RUN_RESULT=../runs/run-bar-chart-validation
